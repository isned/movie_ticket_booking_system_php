...
function modifier_clients(clients $c) {
    $sql = "update clients SET username=?, name=?, password=? , phone=?, email=? where id=?";
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute(array($c->getUsername(), $c->getName(), $c->getPassword(),$c->getPhone(),$c->getEmail(), $c->getId()));
}
}