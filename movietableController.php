...
function comingSoon(){
$query = "select * from movietable where movieRelDate > CURRENT_DATE";
$res=$this->pdo->prepare($query);
$res->execute();
return $res;
}
function getMovietable($movieID){
    $query="SELECT * FROM movietable WHERE movieID = ? ";
    $res = $this->pdo->prepare($query);
    $res->execute(array($movieID));
    $array= $res->fetch();
    return $array;
}
...
function liste_movie($str){
    $query = "SELECT * FROM movieTable where movieGenre='$str' or movieTitle='$str'";
    $res=$this->pdo->prepare($query);
    $res->execute();
    return $res;
    }
